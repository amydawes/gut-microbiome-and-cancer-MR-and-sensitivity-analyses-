...
######################################################################################################################
## PROJECT: MR analysis of the gut microbiome on pancreatic cancer using PRACTICAL, UK Biobank and FinnGen
## Script: Extraction of FGFP microbiome GWAS results based on a defined set of SNPs
## Date: 25/09/26
######################################################################################################################
## Working on Blue Pebble
## Set working directory
cd /path/to/directory/hpc

## Made a flat text file called "prostate_cancer_snps.txt" with the rsids to extract and copied over to HPC
dos2unix prostate_cancer_snps.txt
head prostate_cancer_snps.txt

## Define SNPLIST that is in your working directory
SNPLIST=/path/to/directory/hpc/prostate_cancer_snps.txt

## Define the directory that holds the FGFP GWAS Sum.Stats. files
DATADIR=/path/to/full_fgfp_summary_statistics/fgfp/

## Make a directory in the result folder to hold the data
mkdir results

## Define the directory to place the results
OUTDIR=/path/to/directory/hpc/results
